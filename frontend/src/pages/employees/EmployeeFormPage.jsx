...
import { usersApi, employeesApi, departmentsApi, designationsApi } from '../../api/index.js'
...
export default function EmployeeFormPage() {
  ...
  const userMut = useMutation({
    ...
    onSuccess: (res) => {
      ...
      setStep(2)
      toast.success('User account created. Now fill employee details.')
    },
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500">Step {step} of 2 — {step === 1 ? 'Create User Account' : 'Employee Details'}</p>
      </div>

      {/* Step Indicator */}
      <div className="flex items-center gap-3">
        {[{ n: 1, label: 'User Account' }, { n: 2, label: 'Employee Details' }].map(({ n, label }) => (
          <div key={n} className="flex items-center gap-2">
            <div className={`flex h-7 w-7 items-center justify-center rounded-full text-sm font-semibold ${n <= step ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-500'}`}>{n}</div>
            <span className={`text-sm ${n === step ? 'font-medium text-gray-900' : 'text-gray-500'}`}>{label}</span>
            {n < 2 && <div className={`h-px w-16 ${step > n ? 'bg-primary-600' : 'bg-gray-200'}`} />}
          </div>
        ))}
      </div>
          ...
          <form onSubmit={handleEmpSubmit} className="space-y-6">
            ...
            <div className="flex justify-between pt-2">
              ...
              <button type="submit" disabled={empMut.isPending} className="rounded-md bg-primary-600 px-5 py-2 text-sm font-semibold text-white hover:bg-primary-500 disabled:opacity-50">
                {empMut.isPending ? 'Saving…' : 'Create Employee'}
              </button>
            </div>
          </form>
        </div>
