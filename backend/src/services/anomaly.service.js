...
export async function detectAnomalies(organisationId, { months = 3 } = {}) {
  ...
  for (const emp of employees) {
    ...
    const monFriAbsent = monFriRecords.filter(a => a.status === "ABSENT" || a.status === "LEAVE").length;
    if (monFriRecords.length >= 4 && (monFriAbsent / monFriRecords.length) > 0.3) {
      anomalies.push({
        type: "FREQUENT_MON_FRI_ABSENCE",
        severity: "MEDIUM",
        employee: { id: emp.id, name: `${emp.firstName} ${emp.lastName}`, code: emp.employeeCode, department: emp.department?.name },
    ...
    const recentRecords = attendance.filter(a => new Date(a.date) >= twoWeeksAgo);
    const olderRecords = attendance.filter(a => new Date(a.date) < twoWeeksAgo);

    if (recentRecords.length >= 5 && olderRecords.length >= 10) {
      const recentPresent = recentRecords.filter(a => ["PRESENT", "WFH"].includes(a.status)).length / recentRecords.length;
      const olderPresent = olderRecords.filter(a => ["PRESENT", "WFH"].includes(a.status)).length / olderRecords.length;
    ...
    const leaves = await prisma.leaveApplication.findMany({
      where: { employeeId: emp.id, startDate: { gte: cutoff }, totalDays: 1, status: "APPROVED" },
    });
    if (leaves.length > 5) {
      anomalies.push({
